...
function AdminBookSearch({ selectStyle, bookTypeOptions, categoryOptions }) {
    ...
    const getBookCountQuery = useQuery(
        ["getBookCountQuery", searchBooksQuery.data], // 검색한 결과 후에 카운트를 가져옴
    ...
    const handleCheckOnChange = (e) => {
       const bookId = parseInt(e.target.value); // input안에 들어있는 value = 무조건 string 이기때문에 checkBox = input , book.bookId = int
       setBookList(() => 
            bookList.map(book => {
    ...
    return (
        <div>
            ...
            <div>
                {
                    !getBookCountQuery.isLoading && // 데이터를 다 불러오면 로딩
